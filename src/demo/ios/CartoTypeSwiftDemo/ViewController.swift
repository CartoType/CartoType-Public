...
//  CartoTypeSwiftDemo
//
//  Copyright © 2016-2026 CartoType Ltd. All rights reserved.
//
...
class ViewController: CartoTypeViewController, CartoTypeCreateRouteAsyncProtocol, UISearchBarDelegate, CLLocationManagerDelegate
{
    ...
    lazy var m_find_button = UIBarButtonItem(title: "Find", style: .plain, target: self, action: #selector(onFindButton))
    lazy var m_view_button = UIBarButtonItem(title: "View", style: .plain, target: self, action: #selector(onViewButton))
    lazy var m_route_button = UIBarButtonItem(title: "Route", style: .plain, target: self, action: #selector(onRouteButton))
    lazy var m_navigate_button = UIBarButtonItem(title: "Start", style: .plain, target: self, action: #selector(onNavigateButton))
    lazy var m_help_button = UIBarButtonItem(title: "Help", style: .plain, target: self, action: #selector(onHelpButton))
    ...
    init(aFrameWork: CartoTypeFramework!, aBounds: CGRect)
        ...
        if (m_location_manager.authorizationStatus != .authorizedAlways &&
            m_location_manager.authorizationStatus != .authorizedWhenInUse)
            {
    ...
    override func onLongPress(_ aPointInDegrees: CartoTypePoint)
        ...
        let action_sheet = UIAlertController(title: "Map Options", message: nil, preferredStyle: .actionSheet)
        
        if (m_pushpin_id != 0)
            {
            action_sheet.addAction(UIAlertAction(title: "Delete pin", style: .destructive, handler:
                { _ in
                self.deletePushPin()
                }))
            }
            ...
            {
            action_sheet.addAction(UIAlertAction(title: "Insert pin", style: .default, handler:
                { _ in
                self.insertPushPin()
                }))
            }
            
        action_sheet.addAction(UIAlertAction(title: "Start here", style: .default, handler:
            { _ in
            self.setRouteStart()
            }))
            
        action_sheet.addAction(UIAlertAction(title: "End here", style: .default, handler:
            { _ in
            self.setRouteEnd()
            }))
            
        action_sheet.addAction(UIAlertAction(title: "Cancel", style: .cancel, handler:
            { _ in
            }))
            
        // Configure popover for iPad to point exactly at the tapped map location
        if let popover = action_sheet.popoverPresentationController
            {
            popover.sourceView = self.view
            popover.sourceRect = CGRect(x: p.x / m_ui_scale, y: p.y / m_ui_scale, width: 1, height: 1)
            }
    ...
    func handler(_ aResult: CtResult,route aRoute: CartoTypeRoute!)
        ...
        DispatchQueue.main.async
            ...
            switch (errorcode)
                {
                case CtError.none:
                    self.m_framework.useRoute(aRoute)
                    self.stopNavigating()
    ...
    func calculateAndDisplayRoute()
        ...
        cs.append(start)
        cs.append(end)
            let result = m_framework.createRouteAsync(self, profile: m_framework.routeProfile(), coordSet: cs, override: true)
        if (result != 0)
            {
    ...
    func updateRouteStatus()
        {
        let have_route = m_framework.route != nil
    ...
    func setBuiltInProfile(_ aIndex : Int32)
        {
        m_framework.setBuiltInRouteProfile(aIndex)
        calculateAndDisplayRoute()
        }
    ...
    func find(_ aPlace : String)
        ...
        if (!aPlace.isEmpty)
            ...
            find_param.condition = "@feature_type notin { 'bou', 'aae', 'aaf', 'aag', 'aah', 'aai', 'aaj', 'plf', 'bsp' }"
    ...
    @objc func onRouteButton()
        {
        let route_profile_name = m_framework.routeProfile().name!
        let alert = UIAlertController(title: "Route options", message: "The current profile is '\(route_profile_name)'.", preferredStyle: .alert)
        ...
        if (m_framework.route != nil)
            ...
            alert.addAction(UIAlertAction(title: "Delete Route", style: .default, handler:
                { _ in
                self.stopNavigating()
                self.m_framework.deleteRoute()
                self.m_navigate_button.isEnabled = false
                }))
            }
        
        let built_in_profile_count = m_framework.builtInRouteProfileCount
        for i in 0 ..< built_in_profile_count
            {
            let p = m_framework.builtInRouteProfile(i)!
            let name = p.name!
            if (route_profile_name != name)
                {
                alert.addAction(UIAlertAction(title: "Use profile '\(name)'", style: .default, handler:
                    { _ in
                    DispatchQueue.global(qos: .background).asyncAfter(deadline: .now(), execute: { self.setBuiltInProfile(i) })
                    }))
                }
            }
            
        if built_in_profile_count == 0
            {
            if (route_profile_name != "car")
                {
    ...
    @objc func onViewButton()
        ...
        else if (!m_navigating && m_framework.route != nil)
            {
