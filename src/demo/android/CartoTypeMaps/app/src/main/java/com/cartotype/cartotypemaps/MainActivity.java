...
public class MainActivity extends AppCompatActivity implements LocationListener
    ...
    protected void onCreate(Bundle savedInstanceState)
        ...
        m_style_filename = getAssetPseudonym("standard.ctstyle");
        if (m_style_filename == null)
            {
            Log.d("CartoType","failed to open style sheet file");
            return;
            }
        m_font_filename = getAssetPseudonym("DejaVuSans.ttf");
        if (m_font_filename == null)
            {
            Log.d("CartoType","failed to open font file");
            return;
            }
        String map_filename = getAssetPseudonym("isle_of_wight.ctm1");
        if (map_filename == null)
            {
            Log.d("CartoType","failed to open map file");
            return;
            }
        createMap(map_filename,"");
    ...
    public boolean onOptionsItemSelected(MenuItem aItem)
        ...
        if (id == R.id.file_open_app)
            {
            String dir = getExternalFilesDir(null).getPath();
            chooseFile(getExternalFilesDir(null).getPath(), false);
            return true;
            }

        if (id == R.id.file_open_picker)
            {
            openFilePicker();
            return true;
            }
    ...
    private void createMap(String aMapFilename,String aKey)
        ...
        // Load extra fonts.
        String font1 = getAssetPseudonym("DejaVuSans-Bold.ttf");
        int e = m_framework.loadFont(font1);
        String font2 = getAssetPseudonym("DejaVuSerif.ttf");
        if (e == 0)
            e = m_framework.loadFont(font2);
        String font3 = getAssetPseudonym("DejaVuSerif-Italic.ttf");
        if (e == 0)
            e = m_framework.loadFont(font3);
        if (e != 0)
            {
            Log.d("CartoType","failed to open font file: " + Error.string(e));
            }
    ...
    private void openFilePicker()
        ...
        intent.setType("*/*");
        m_open_document_launcher.launch(intent);
        }
    ...
    /**
    Converts a path to an asset file, which must not be compressed, to
    a file pseudonym encoding a file descriptor, start position and length,
    representing the asset file within the asset archive. The pseudonym has the
    form ':FD,START,LENGTH'.

    CartoType's file stream system understands these pseudonyms.

    The point of doing this is to avoid having copy the asset data into a
    separate file, which was done formerly.
    */
    private String getAssetPseudonym(String aAssetPath)
        {
        ...
        try
            {
            afd = getAssets().openFd(aAssetPath);
    ...
    private void initFilePicker()
        ...
        m_open_document_launcher = registerForActivityResult
                (
                        ...
                        if (data != null)
                            ...
                            if (uri != null)
                                ...
                                try
                                    ...
                                    if (pfd != null)
                                        {
                                        // Get the integer file descriptor and take ownership of it.
                                        int fd = pfd.detachFd();

                                        /*
                                        Use a pseudonym to open the file from the descriptor rather than from a real path.
                                        CartoType's input file stream system recognises names of the form ':N'
                                        as referring to the open file descriptor N.
                                        */
                                        String pseudonym = ":" + fd;
                                        createMap(pseudonym,"");
                                        }
                                        ...
                                        {
                                        throw new IOException("Could not get a ParcelFileDescriptor from the URI.");
                                        }
                                    }
                                catch (IOException e)
                                    {
    ...
    private ActivityResultLauncher<Intent> m_open_document_launcher;
    }
