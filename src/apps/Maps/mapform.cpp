...
MapForm::MapForm(QWidget* aParent,MainWindow& aMainWindow,
                 const std::vector<QString>& aMapFileNameArray,
                 const MapSettings& aSettings):
    ...
    for (size_t i = 1; i < aMapFileNameArray.size(); i++)
        ...
        if (error)
            ...
            m_filename_array.push_back(aMapFileNameArray[i]);
        }

    // Apply the settings.
    SetMetricUnits(aSettings.m_metric_units);
    SetNightMode(aSettings.m_night_mode);
    SetMonochrome(aSettings.m_monochrome);
    EnableDebugLayers(aSettings.m_debug_layers);
    EnableFixedLabels(aSettings.m_fixed_labels);
    SetDraw3DBuildings(aSettings.m_draw_3d_buildings);
    EnableDrawLegend(aSettings.m_draw_legend);
...
void MapForm::LeftButtonUp(int32_t aX,int32_t aY,bool aShift)
    ...
    if (m_simulate_routing && HasRoute() && aShift && m_map_drag_offset.X == 0 && m_map_drag_offset.Y == 0)
        ...
        CartoType::Result error;
        nav.Position = m_framework->ConvertPoint(error,{double(aX),double(aY)},CartoType::CoordType::Display,CartoType::CoordType::Degree);
        if (!error)
            error = m_framework->Navigate(nav);
        if (error)
...
void MapForm::ShowRouteDialog(int32_t aX,int32_t aY)
    ...
    CartoType::Result error;
    p = m_framework->ConvertPoint(error,p,CartoType::CoordType::Display,CartoType::CoordType::Degree);
    ...
    if (!error)
        ...
        if (m_framework->MetricUnits())
            snprintf(buffer,sizeof(buffer)," (altitude = %dm)",CartoType::Round(alt[0]));
        else
            snprintf(buffer,sizeof(buffer)," (altitude = %dft)",CartoType::Round(alt[0] / 0.3048));
    ...
    rp.Point = m_framework->ConvertPoint(error,{double(aX),double(aY)},CartoType::CoordType::Display,CartoType::CoordType::Map);

    // Set the heading to that of the nearest road if it is less than 10 metres away.
    ...
    if (info.Distance < 10)
        {
        rp.Heading = info.HeadingInDegrees;
    ...
    if (route_dialog.m_ui->setStartOfRoute->isChecked()) // set start of route
        ...
        if (pushpin)
            ...
            {
            auto p = m_framework->ConvertPoint(error,CartoType::PointFP(aX,aY),CartoType::CoordType::Display,CartoType::CoordType::Map);
            x = p.X;
            y = p.Y;
            }
        if (m_pushpin_glyph)
...
void MapForm::SetMonochrome(bool aSet)
    {
    m_framework->SetMonochrome(aSet);
    CreateLegend();
    update();
    }

bool MapForm::Monochrome() const
    {
    return m_framework->Monochrome();
    }
...
void MapForm::SetUseCustomStyleSheet(bool aEnable)
    ...
    if (aEnable)
        ...
        m_framework->SetStyleSheet(m_saved_style_sheet,0);
    CreateLegend();
    update();
    }
