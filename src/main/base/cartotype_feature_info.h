...
Copyright (C) 2022-2026 CartoType Ltd.
...
enum class FeatureType: uint16_t
    ...
    */
    ...
    PowerPole = FeatureTypeCode('p','w','p'),
    PowerStation = FeatureTypeCode('p','w','s'),
    PowerTower = FeatureTypeCode('p','w','t'),
    Quarry = FeatureTypeCode('q','a','r'),
    Quay = FeatureTypeCode('q','a','y'),
    ...
    WildernessHut = FeatureTypeCode('w','i','l'),
...
class FeatureInfo
    ...
    uint32_t WeightIndex(bool aCustomRouteType) const;
    ...
    */
    ...
    Route profiles also allow custom route types in the range 32...64. The value RouteProfile::RouteTypeCount = 64 should be used for the extended range.
    */
    ...
    */
    ...
    /** A mask to select the discriminator field. */
    static constexpr uint32_t KDiscriminatorMask = 3;
    /** The discriminator value indicating a non-route. */
    static constexpr uint32_t KDiscriminatorNonRoute = 0;
    /** The discriminator value indicating a route object that is a bridge. */
    static constexpr uint32_t KDiscriminatorBridge = 1;
    /** The discriminator value indicating a route object that is a tunnel. */
    static constexpr uint32_t KDiscriminatorTunnel = 2;
    /** The discriminator value indicating a route object that is neither a bridge nor a tunnel. */
    static constexpr uint32_t KDiscriminatorRoute = 3;
    /** The amount by which the level field is shifted. */
    static constexpr uint32_t KLevelShift = 2;
    /** A mask to select the level field. */
    static constexpr uint32_t KLevelMask = 15 << KLevelShift;
    /** The amount by which the type field is shifted. */
    static constexpr uint32_t KTypeShift = 6;

    // route-specific constants
    /** A mask to select the type field for routes. */
    static constexpr uint32_t KRouteTypeMask = 31 << KTypeShift;
    /** A flag indicating that this route is one-way. */
    static constexpr uint32_t KRouteOneWayFlag = 1 << 11;
    /** A flag indicating that this route's driving side is the left. */
    static constexpr uint32_t KRouteDriveOnLeftFlag = 1 << 12;
    /** A flag indicating that this route is part of a roundabout. */
    static constexpr uint32_t KRouteRoundaboutFlag = 1 << 13;
    /** A flag indicating that a toll applies to this route. */
    static constexpr uint32_t KRouteTollFlag = 1 << 14;
    /** The amount by which the speed limit field is shifted in routes. */
    static constexpr uint32_t KRouteSpeedLimitShift = 15;
    /** A mask to select the speed limit field in routes. */
    static constexpr uint32_t KRouteSpeedLimitMask = 255 << KRouteSpeedLimitShift;
    /** The amount by which the gradient field is shifted in routes. */
    static constexpr uint32_t KRouteGradientShift = 23;
    /** A mask to select the gradient field in routes. */
    static constexpr uint32_t KRouteGradientMask = 7 << KRouteGradientShift;
    /** A flag indicating that the gradient in the direction of this route is downward. */
    static constexpr uint32_t KRouteGradientDirectionFlag = 4 << KRouteGradientShift;

    // non-route-specific constants
    /** A mask to select the type field for non-routes. */
    static constexpr uint32_t KNonRouteTypeMask = 32767 << KTypeShift;
    /** The amount by which the subtype field is shifted in non-routes. */
    static constexpr uint32_t KNonRouteSubTypeShift = 21;
    /** A mask to select the subtype field for non-routes. */
...
std::string RouteTypeName(uint32_t aRouteType);
FeatureType FeatureTypeFromName(const MString& aName);
uint32_t RouteTypeFromName(const MString& aName);
