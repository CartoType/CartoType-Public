...
class Result
    ...
    /** Returns a short English description of this result. */
...
/**
The code KErrorIo is used for unexpected read or write
errors on files or other data streams.
It has now largely been superseded by the KErrorSystem range of error codes,
which contain the system errno value.
*/
...
constexpr Result KErrorRouteOutsideMap = 72;

/** An attempt was made to load a map which is already loaded. */
constexpr Result KErrorMapAlreadyLoaded = 73;

/** The number of standard error codes. */
constexpr int32_t KStandardErrorCodeCount = 74;

/** Returns a short English description of a result, given its code. */
std::string ResultString(uint32_t aResultCode);

/** Returns a short English description of a result, given its code. A synonym of ResultString. */
inline std::string ErrorString(uint32_t aErrorCode) { return ResultString(aErrorCode); }
...
constexpr uint32_t KErrorSystem = 0x40000000;
