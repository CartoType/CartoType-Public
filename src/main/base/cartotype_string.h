...
class MString
    ...
    */
    ...
    std::basic_string<char16_t> CreateUtf16String() const;
    std::string CreateUtf8String() const;
    std::string PercentEncode() const;
    ...
    Text Token(size_t aIndex,const MString* aSeparators = nullptr,const MString* aSingleCharacterTokens = nullptr) const;
    ...
    */
    ...
    void Append(const MString& aString)
        ...
        auto source = aString.Data();
        ResizeBufferAndFixPointer(iLength + aString.iLength,source);
        std::memmove(WritableText() + old_length,source,(iLength - old_length) * 2);
        }
    ...
    void Set(const MString& aString)
        {
        Set(aString.Data(),aString.iLength);
        }
    ...
    */
    ...
    void Append(const uint16_t* aText,size_t aLength = npos)
        ...
        size_t old_length = iLength;
        ResizeBufferAndFixPointer(iLength + aLength,aText);
        memmove(WritableText() + old_length,aText,(iLength - old_length) * 2);
        }
    ...
    void Set(const uint16_t* aText,size_t aLength = npos)
        ...
        if (aText >= Data() && aText < Data() + iLength)
            {
            // Internal pointer: Move data first then resize.
            assert(aText + aLength <= Data() + iLength);
            size_t offset = aText - Data();
            std::memmove(WritableText(),WritableText() + offset,aLength * 2);
            ResizeBuffer(aLength);
            }
        else
            {
            ResizeBuffer(aLength);
            std::memmove(WritableText(),aText,iLength * 2);
            }
        }
    ...
    */
    ...
    // treating the string as a file path
    Text PathExtension() const noexcept;
    Text PathDirectory() const noexcept;
    Text PathFileName() const noexcept;
    void SetPathExtension(const char* aExtension);
    ...
    */
    ...
    /** Resizes the text buffer and corrects a pointer if it points inside the buffer, because the buffer's address might have changed. */
    void ResizeBufferAndFixPointer(size_t aNewLength,const uint16_t*& aPointer)
        {
        bool source_is_internal = aPointer >= Data() && aPointer < Data() + iLength;
        size_t offset = source_is_internal ? (aPointer - Data()) : 0;
        ResizeBuffer(aNewLength);
        if (source_is_internal)
            aPointer = Data() + offset;
        }
...
class Text: public MString
    ...
    /** Constructs a Text object from a null-terminated string. */
    ...
    /** Constructs a Text object from a pointer and a length. */
    ...
    /** Constructs a Text object from an MString object. */
    Text(const MString& aString) noexcept : MString(aString.Length()), iText(aString.Data()) { }
    /** Constructs a Text object from null-terminated UTF-16 text. */
