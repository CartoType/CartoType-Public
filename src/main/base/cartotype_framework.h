...
Copyright (C) 2012-2026 CartoType Ltd.
...
class Framework: public MNavigatorObserver
    ...
    */
    class Param
        ...
        FilePolicy MapFilePolicy = FilePolicy::Default;
        };
    ...
    Result LoadMap(const String& aMapFileName,const std::string* aKey = nullptr,FilePolicy aPolicy = FilePolicy::Default);
    Result LoadMapsAutomatically(const TileParam& aParam);
    void CancelAutomaticMapLoading();
    Result LoadMapAsync(LoadMapAsyncCallBack aCallback,const String& aMapFileName,const std::string& aKey,FilePolicy aPolicy = FilePolicy::Default);
    ...
    Result SetStyleSheet(std::shared_ptr<CartoTypeCore::StyleSheetData> aStyleSheetData,size_t aIndex = 0);
    Result SetStyleSheet(const CartoTypeCore::StyleSheetDataArray& aStyleSheetDataArray,const StringDictionary* aStyleSheetVariables = nullptr,const BlendStyleSet* aBlendStyleSet = nullptr);
    ...
    std::shared_ptr<CartoTypeCore::StyleSheetData> StyleSheetData(size_t aIndex) const;
    ...
    [[nodiscard]] PointFP ConvertPoint(Result& aError,const PointFP& aPoint,CoordType aFromCoordType,CoordType aToCoordType) const;
    Result ConvertPoint(double& aX,double& aY,CoordType aFromCoordType,CoordType aToCoordType) const;
    [[nodiscard]] PointFP ConvertPointInternal(const PointFP& aPoint,CoordType aFromCoordType,CoordType aToCoordType) const;
    ...
    // terrain heights
    std::vector<double> Heights(Result& aError,const CoordSet& aCoordSet,CoordType aCoordType) const;
    ...
    Result DisplayRouteChoices(const std::vector<std::shared_ptr<CartoTypeCore::Route>>& aRouteArray);
    ...
    Result AddTrafficEvent(uint64_t& aId,const TrafficEvent& aTrafficEvent);
    ...
    Result AddLineSpeedLimit(uint64_t& aId,const LRGeometry& aLine,double aSpeed,uint32_t aVehicleTypes);
    Result AddClosedLineSpeedLimit(uint64_t& aId,const LRGeometry& aLine,double aSpeed,uint32_t aVehicleTypes);
    Result AddForbiddenArea(uint64_t& aId,const Geometry& aPolygon);
    Result DeleteTrafficEvent(uint64_t aId);
    Result ClearTrafficInfo();
    ...
    UpdateTrafficResult UpdateTrafficFromDatex2(const std::string& aXmlText);
    ...
    Transform3D MapTransform3D() const;
    CartoTypeCore::MapTransform MapTransform2D() const;
    Transform3D PerspectiveTransform() const;
    /** Returns the FrameworkEngine object used by this framework. For internal use only. */
    CartoTypeCore::FrameworkEngine& FrameworkEngine() const { return *iEngine; }
    /** Returns the CEngine object used by this framework. For internal use only. */
    std::shared_ptr<CEngine> Engine() const;
    /** Returns the FrameworkMapDataSet object owned by this framework. For internal use only.  */
    FrameworkMapDataSet& MapDataSet() const { return *iMapDataSet; }
    ...
    Result Construct(const Param& aParam,
                     ...
                     const CartoTypeCore::StyleSheetDataArray* aStyleSheetDataArray = nullptr,
                     const StringDictionary* aVariableDictionary = nullptr,
                     const CartoTypeCore::BlendStyleSet* aBlendStyleSet = nullptr);
    ...
    std::vector<RoutePointInternal> CreateRoutePointArray(const RouteCoordSet& aRouteCoordSet) const;
    double ScaleForSpeed(double aSpeedInKph) const;
    void InsertRouteObject(const MPath& aPath,const MString& aLayer,const CartoTypeCore::RouteProfile& aProfile,int aProfileNumber,
                           double aTime,double aDistance,bool aTravelled,
                           uint64_t& aLineId,uint64_t& aEnvelopeId);
    ...
    void ClearTurns();
    void AddLineSpeedLimitHelper(uint64_t& aId,const LRGeometry& aLine,double aSpeed,uint32_t aVehicleTypes,bool aIsClosed);
