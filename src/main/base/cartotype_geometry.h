...
Copyright (C) 2019-2026 CartoType Ltd.
...
template<class point_t> class GeneralGeometry
    ...
    [[nodiscard]] operator Outline() const
        {
        Outline outline;
        OutlinePoint p;
    ...
    [[nodiscard]] CartoTypeCore::CoordType CoordType() const { return m_coord_type; }
    /** Returns the number of contours. */
    [[nodiscard]] size_t ContourCount() const { return m_contour_array.size(); }
    /** Returns a contour selected by its index as a vector of points. */
    [[nodiscard]] const contour_t& ContourByIndex(size_t aIndex) const { return m_contour_array[aIndex]; }
    ...
    [[nodiscard]] bool ContourIsOuter(size_t aIndex) const { return Area(m_contour_array[aIndex]) > 0; }
    /** Returns the number of points in a given contour. */
    [[nodiscard]] size_t PointCount(size_t aContourIndex) const { return m_contour_array[aContourIndex].size(); }
    /** Returns a point identified by its contour index and point index. */
    [[nodiscard]] const point_t& Point(size_t aContourIndex,size_t aPointIndex) const { return m_contour_array[aContourIndex][aPointIndex]; }
    /** Returns a non-const reference to a point identified by its contour index and point index. */
    [[nodiscard]] point_t& Point(size_t aContourIndex,size_t aPointIndex) { return m_contour_array[aContourIndex][aPointIndex]; }
    /** Returns true if the geometry is empty. */
    [[nodiscard]] bool IsEmpty() const { return m_contour_array[0].empty(); }
    ...
    [[nodiscard]] WritableCoordSet CoordSet(size_t aContourIndex)
        {
        return WritableCoordSet(m_contour_array[aContourIndex]);
        }
    /** Returns a coordinate set referring to this geometry. */
    [[nodiscard]] CartoTypeCore::CoordSet CoordSet(size_t aContourIndex) const
        {
...
class Geometry: public GeneralGeometry<OutlinePointFP>
    ...
    [[nodiscard]] PathIntersectionType IntersectionType(const MapObject& aMapObject) const;
    [[nodiscard]] PathIntersectionInfo IntersectionInfo(const MapObject& aMapObject) const;
    [[nodiscard]] std::vector<PathIntersectionType> IntersectionType(std::function<const MapObject*(size_t)> aObject,size_t aObjectCount) const;
    [[nodiscard]] std::vector<PathIntersectionType> IntersectionType(const MapObjectArray& aMapObjectArray) const;
    [[nodiscard]] std::vector<PathIntersectionInfo> IntersectionInfo(std::function<const MapObject* (size_t)> aObject,size_t aObjectCount) const;
    [[nodiscard]] std::vector<PathIntersectionInfo> IntersectionInfo(const MapObjectArray& aMapObjectArray) const;
    /** Returns true if this geometry intersects, contains or is contained by the map object aMapObject. */
    [[nodiscard]] bool Intersects(const MapObject& aMapObject) const
        {
    ...
    [[nodiscard]] bool Contains(const MapObject& aMapObject) const { return IntersectionType(aMapObject) == PathIntersectionType::Contains; }
    /** Returns true if this geometry is contained by the map object aMapObject. */
    [[nodiscard]] bool Contained(const MapObject& aMapObject) const { return IntersectionType(aMapObject) == PathIntersectionType::Contained; }
    [[nodiscard]] Geometry Clip(ClipOperation aClipOperation,const MapObject& aMapObject) const;
    [[nodiscard]] std::vector<Geometry> Clip(ClipOperation aClipOperation,const MapObjectArray& aMapObjectArray) const;
    [[nodiscard]] std::vector<Geometry> Clip(ClipOperation aClipOperation,std::function<const MapObject* (size_t)> aObject,size_t aObjectCount) const;
    [[nodiscard]] static Geometry Envelope(const MapObject& aMapObject,double aOffsetInMeters);
