...
class RouteSegment
    ...
    Uses aProjection to convert the points to longitude (x) and latitude (y).
    */
...
class RouteProfile
    ...
    /** Copies all weights from one route type to another. Intended to make it easier to create custom route profiles. */
    Result CopyWeights(size_t aSourceType,size_t aDestType);
    ...
    /** Returns the index for a custom type 0...31 in the speed, bonus and restriction override arrays. This function just adds 32, but may help readability. */
    static size_t CustomType(size_t aIndex) { assert(aIndex < 32); return aIndex + 32; }

    /** The size of the speed, bonus ands restriction override arrays, which are indexed by the route feature types from 0 to 31 and custom route types 32 to 63. */
    static constexpr size_t RouteTypeCount = 64;
    ...
    std::vector<double> Speed = std::vector<double>(RouteTypeCount,0.0);
    ...
    */
    std::vector<double> Bonus = std::vector<double>(RouteTypeCount,0.0);
    ...
    RestrictionOverride[RouteProfile::Track] to FeatureInfo::KRouteAccessVehicle.
    */
    std::vector<uint32_t> RestrictionOverride = std::vector<uint32_t>(RouteTypeCount,0);
    ...
    */
    std::vector<double> GradientSpeed = std::vector<double>(FeatureInfo::KGradientCount,0);

    /** Bonuses applied to roads with a particular gradient. */
    std::vector<double> GradientBonus = std::vector<double>(FeatureInfo::KGradientCount,0);

    /**
    Flags indicating which roads are affected by gradient speeds and bonuses: by default: everything except steps and ferries.
    Each bit is (1 << route type).
    */
    uint64_t GradientFlags = UINT64_MAX & ~((1 << Steps) | (1 << PassengerFerry) | (1 << VehicularFerry));
    };
...
class Route
    ...
    Uses aProjection to project the points from longitude (x) and latitude (y).
    */
...
class NavigationData
    ...
    double Time = 0;
    /** Position in degrees longitude (x) and latitude (y). */
    PointFP Position;
...
class NavigatorParam
    ...
    If false, the vehicle position and speed are updated but other behavior is the same as if there is no route.
    */
...
enum class LRType
    ...
    Circle,
    /** A rectangle aligned to the grid of latitude and longitude: it is defined using two points with X=longitude and Y=latitude at opposite corners. */
    Rectangle,
...
enum class LRFunctionalRoadClass
    {
    Main = 0,
    First = 1,
    Second = 2,
...
class LRPoint
    ...
    LRFunctionalRoadClass FRC = LRFunctionalRoadClass::Unknown;
    /** Form of way at position. */
    LRFormOfWay FOW = LRFormOfWay::Unknown;
    /** Forward bearing in degrees (0...360, clockwise, 0 = north) at position. */
    double Bearing = 0;
    /** True if the bearing is known. */
    bool BearingKnown = false;
    ...
    LRFunctionalRoadClass LowestRoadClassToNextPoint = LRFunctionalRoadClass::Unknown;
    };

/** A series of LRPoint objects. */
using LRGeometry = std::vector<LRPoint>;

/** Parameters used for a traffic event location reference. */
class LocationRef
    {
    public:
    LocationRef() = default;

    /** Creates a location reference with the specified reference type. */
    ...
    LRType Type = LRType::Line;
    /** The arbitrary ID of the location reference. */
    String Id;
    /** The geometry: a point or points. */
    LRGeometry Geometry;
    ...
    CartoTypeCore::RoadOrientation RoadOrientation = CartoTypeCore::RoadOrientation::None;
    /** The offset forward from the first location in meters; 'positive offset' in OpenLR terminology. */
    double ForwardOffset = 0;
    /** The offset backward from the last location in meters; 'negative offset' in OpenLR terminology. */
    double BackwardOffset = 0;
    };

/** Information needed when adding a traffic information event. */
class TrafficEvent
    {
    public:
    /** Creates a TrafficEvent from a DATEX II XML situation record. */
    static bool CreateFromDatex2Xml(const std::string& aSituationRecord,TrafficEvent& aTrafficEvent);
    
    /** The traffic information. */
    CartoTypeCore::TrafficInfo TrafficInfo;
    /** The location reference. */
    CartoTypeCore::LocationRef LocationRef;
    };

/** The return value of the Framework function UpdateTrafficFromDatex2. */
class UpdateTrafficResult
    {
    public:
