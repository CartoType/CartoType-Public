...
class Legend: public MFrameworkObserver
    {
    public:
    static std::unique_ptr<Legend> New(Result& aError,Framework& aFramework,uint32_t aStyle = KEmptyStyle);
    static std::unique_ptr<Legend> Copy(Result& aError,const Legend& aOther);
    ~Legend();
