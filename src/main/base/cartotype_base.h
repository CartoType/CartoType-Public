...
template<class T> class Point3
    ...
    T DotProduct(const Point3<T>& aOther) const
        {
        return X * aOther.X + Y * aOther.Y + Z * aOther.Z;
        }
    /** Returns the cross product of this vector and another vector. */
    Point3<T> CrossProduct(const Point3<T>& aOther) const
        {
        return Point3<T>
            {
...
/** A value representing an invalid height relative to sea level. */
constexpr double KInvalidHeight = -32768.0;

/** A point on a height profile. */
class HeightProfilePoint
    {
    public:
    /** The less-than operator, which compares on height. */
...
/** File and data types of interest to CartoTypeCore. */
enum class FileType
    {
    ...
    GPX,
    /** In-memory map databases. */
    IN_MEMORY_MAP_DB,
    /** Online CTM1 data. */
    CTM1_ONLINE,
    /** Online Mapbox vector tile data. */
    MVT_ONLINE,
...
/**
The anchor position, width and insets for notices like the legend or scale bar.
...
class NoticePosition
    ...
    NoticePosition(NoticeAnchor aAnchor,double aWidth,const char* aWidthUnit,double aXInset = 0,const char* aXInsetUnit = nullptr,double aYInset = 0,const char* aYInsetUnit = nullptr);
    /** Returns the anchor position. */
    NoticeAnchor Anchor() const { return m_anchor; }
    /** Returns the width. */
    double Width() const { return m_width; }
    /** Returns the unit (e.g., "mm") used for the width. */
    const char* WidthUnit() const { return m_width_unit.data(); }
    ...
    private:
    NoticeAnchor m_anchor = NoticeAnchor::TopLeft;
    double m_width = 0;
    std::array<char,3> m_width_unit = { };
    double m_x_inset = 0;
...
/** Size of a scaled degree in map units. */
constexpr int32_t KScaledDegree = 65536 * 32;
...
/** The minimum tile zoom level. */
constexpr int32_t KMinTileZoomLevel = 0;

/** The maximum tile zoom level. */
constexpr int32_t KMaxTileZoomLevel = 21;
...
/** Zoom, X and Y coordinates for a tile in any system. */
class TileXYZ
    {
    public:
    ...
    int32_t Y = 0;
    /** The zoom level of the tile; -1 indicates an invalid tile. */
    int32_t Zoom = -1;
    };

/** A filename with the tile it represents; used when loading and unloading tile files automatically. */
class TileFileInfo
    {
    public:
    /** The filename. */
    std::string FileName;
    /** The tile specifiction. */
    TileXYZ TileSpec;
    };
